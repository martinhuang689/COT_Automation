...
# Handle colorama
if ensure_module_installed('colorama'):
    import colorama
...
# Handle pyperclip
if ensure_module_installed('pyperclip'):
    import pyperclip
...
def parse_value(value_str):
    ...
    if value_str.startswith("<"):
        ...
        lower, upper = map(int, value_str.split("¡V"))
        return (lower, upper)
    else:
        val = int(value_str)
        return (val, val)
...
def print_results(results, outcomes):
    ...
    if pyperclip_available:
        ...
        print("Note: pyperclip is not installed. Please copy the NRICPassportNo manually from the output above.")